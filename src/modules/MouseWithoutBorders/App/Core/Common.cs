...
#pragma warning restore SA1307
    ...
    internal static void SendEnhancedDragChunk(ID destination, Guid dragId, int index, int total, byte[] payload)
    {
        ArgumentNullException.ThrowIfNull(payload);
        if (payload.Length > 20)
        {
            throw new ArgumentOutOfRangeException(nameof(payload), "Enhanced drag payload chunks are limited to 20 bytes.");
        }

        // A big MWB DATA packet has 48 bytes after its header. The 28-byte
        // chunk header leaves 20 bytes for UTF-8 manifest data and avoids the
        // old fixed-string truncation behavior.
        var bytes = new byte[48];
        var offset = Package.PACKAGE_SIZE_EX - 48;
        dragId.TryWriteBytes(bytes.AsSpan(0, 16));
        BitConverter.GetBytes(index).CopyTo(bytes, 16);
        BitConverter.GetBytes(total).CopyTo(bytes, 20);
        BitConverter.GetBytes(payload.Length).CopyTo(bytes, 24);
        payload.CopyTo(bytes, 28);

        var package = new DATA { Type = PackageType.EnhancedDragManifestChunk, Src = MachineID, Des = destination, MachineName = MachineName };
        var packageBytes = package.Bytes;
        bytes.CopyTo(packageBytes, offset);
    ...
    internal static IPAddress GetConnectedIPv4AddressFor(string machineName)
    {
        SocketStuff sk = Common.Sk;
        ...
        if (sk != null)
        {
            lock (sk.TcpSocketsLock)
            {
                return sk.TcpSockets.FirstOrDefault(t => t != null && t.Status == SocketStatus.Connected
                        ...
                        && t.MachineName.Equals(machineName, StringComparison.OrdinalIgnoreCase))
                        ?.Address;
            }
        }
