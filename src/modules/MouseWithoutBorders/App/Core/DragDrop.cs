...
internal static class DragDrop
{
    ...
    {
        DragDropStep05Ex(new[] { dragFileName });
    }

    internal static void DragDropStep05Ex(string[] dragFileNames)
    {
        ...
        if (!IsDropping)
        {
            ...
            {
                string[] validPaths = dragFileNames
                    .Where(path => !string.IsNullOrWhiteSpace(path) && (File.Exists(path) || Directory.Exists(path)))
                    .Distinct(StringComparer.OrdinalIgnoreCase)
                    .ToArray();
                if (validPaths.Length > 0)
                {
                    Clipboard.LastDragDropFile = validPaths[0];
                    /*
                    ...
                    DragDropStep06();
                    Logger.LogDebug("DragDropStep05: File dragging: " + string.Join(";", validPaths));
                    _ = NativeMethods.PostMessage(Common.MainForm.Handle, NativeMethods.WM_HIDE_DD_HELPER, (IntPtr)1, (IntPtr)0);
                }
                else
                {
                    Logger.LogDebug("DragDropStep05: No valid dragged files.");
